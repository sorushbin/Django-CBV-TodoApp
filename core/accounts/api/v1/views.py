    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage(
                'email/activation_email.tpl', {'token': token}, 'admin@admin.com', to=[email])
...
class ActivationResendApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage(
            'email/activation_email.tpl', {'token': token}, 'admin@admin.com', to=[user_obj.email])
        EmailThread(email_obj).start()
        return Response({'details': 'user activation resend successfully. Please check your mail box.'}, status=status.HTTP_200_OK)
