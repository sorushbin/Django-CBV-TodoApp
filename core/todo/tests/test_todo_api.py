...
@pytest.fixture
def profile(user):
    return Profile.objects.create(user= user, first_name='mahmoud', last_name='sifi', description='i am teacher', updated_date=datetime.now())


@pytest.fixture
def task(profile):
    return Task.objects.create(user=profile, title='test title', description='test description', done=True)
